...
- Status: ready
- Depends on: B-001, B-002, B-003, B-004, B-005
...
## References
- Decisions: product/decisions/0003, 0004, 0006 (binding); 0005 (protects BYO-key deps)
...
- Remove orphaned locale keys (auth, billing, workspace, telemetry, Google Calendar) from all 9 `src/locales/*/translation.json` files — match by key, verify each key has no remaining `t("...")` caller.
...
- Update `README.md` / `CLAUDE.md` feature lists: no accounts, no cloud sync, Apple Calendar instead of Google, unsigned local builds.
...
## Acceptance criteria
- [ ] All B-001…B-005 grep gates still pass (run them all; they are the regression net):
      `grep -rn "openwhispr\.com\|OPENWHISPR_API_URL\|better-auth\|telemetryEnabled\|googleCalendar\|gcal-" src main.js preload.js` returns nothing.
...
grep -rn "openwhispr\.com\|OPENWHISPR_API_URL\|better-auth\|telemetryEnabled\|googleCalendar\|gcal-" src main.js preload.js && echo FAIL || echo PASS
