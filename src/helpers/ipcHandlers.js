...
class IPCHandlers {
  ...
  setupHandlers() {
    ...
    // Electron does not auto-attach jar cookies on top of explicit provider headers.
    ...
    const fetchRealtimeToken = async (_event, options, { streams } = {}) => {
      const dual = (value) => (streams === 2 ? [value, value] : value);

      if (options.provider === "assemblyai-realtime") {
        if (options.mode !== "byok") {
          const err = new Error("AssemblyAI streaming requires a configured API key.");
          err.code = "NO_API";
          throw err;
        }
        ...
        return dual(data.token);
      }

      if (options.provider === "deepgram-realtime") {
        if (options.mode !== "byok") {
          const err = new Error("Deepgram streaming requires a configured API key.");
          err.code = "NO_API";
          throw err;
        }
